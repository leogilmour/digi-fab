...
const int POUR_TIMEOUT_MILLIS = 15000;
// other timeouts to be added here:
// hot pot timeout?
// stepper timeout?
unsigned long maxHeatingTimeMins = 1; // 1 hour heating time
...
const int water_motor_a_pin = 12;
const int water_motor_b_pin = 13;
int tea_a_in_3 = 14;
int tea_b_in_1 = 27;
int tea_c_in_2 = 26;
int tea_d_in_4 = 25;

bool wind_strainer = false;
int strainer_direction = 1;
bool pour_water = false;

const int stepsPerRevolution = 2048; // change this to fit the number of steps per revolution
Stepper myStepper(stepsPerRevolution, tea_a_in_3, tea_b_in_1, tea_c_in_2, tea_d_in_4);
...
public:
    TeaMakingStateMachine(DCMotor *wm, Boiler *b = nullptr)
        : currentState(IDLE), previousState(IDLE), stateStartTimeMillis(0),
          brewDurationMillis(),
          waterMotor(wm), boiler(b) {}
...
private:
    ...
    void onStateExit(State state)
    {
        ...
        case BREWING_TEA:
            ...
            strainer_direction = strainer_direction * -1;
            wind_strainer = true;
            break;
        case TEA_READY:
            userSetReadyTime = "";
    ...
    void handlePouringWaterState(unsigned long timeInStateMillis)
    {
        ...
        if (timeInStateMillis > pourTimeoutMillis)
        {
...
TeaMakingStateMachine teaMakingMachine(&waterMotor, &boiler);
...
void handleRoot()
{
    ...
    if (targetMinutesToSteep == 0)
    {
        ...
        html += "<form action=\"/api/schedule-tea\" method=\"post\">"
                ...
                "</label>"
                "<div class='grid'>"
                "<input type='time' id='readyTime' name='readyTime' value='08:00' />"
